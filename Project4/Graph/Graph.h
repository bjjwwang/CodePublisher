...
namespace GraphLib
{
	...
	public:
		...
		iterator begin();
		iterator end();
		std::vector<Edge>& getEdge() { return _edges; } //这里获取Edge iterator什么JB
	...
	public:
		...
		Vertex<V, E> GetVertexById(size_t id);

		size_t size();
		template<typename F>
		void dfs(Vertex<V, E>& v, F f);
		std::vector<Vertex<V, E>> & GetAdj() { return adj; }

		std::string toXml(std::string path);
	...
	template<typename V, typename E>
	Vertex<V, E> Graph<V, E>::GetVertexById(size_t id)
	{
		Vertex<V, E> res;
		for (Vertex<V, E> node : adj)
		{
	...
	public:
		...
		static void show(graph& g)
		{
			...
			{
				vertex v = *iter;
				std::cout << "\n  vertex id = " << v.id() << ", Path = " << (v.value()).c_str();
				for (size_t i = 0; i<v.size(); ++i)
				{
					vertex::Edge edge = v[i];
					std::cout << "\n    Dependency points to vertex with id = " << g[edge.first].id();
					std::cout << " and Path = " << g[edge.first].value().c_str();
	...
	template<typename V, typename E>
	std::string Graph<V, E>::toXml(std::string path)
	{
		Graph<V, E> &gr = *this;
		XmlWriter wrg;
		wrg.indent();
		...
		for (auto vert : gr)
		{
			...
			for (auto edge : vert)
			{
				...
				wrg.addAttribute("edge", edge.second);
				wrg.end();
			}
			wrg.end();
		}
