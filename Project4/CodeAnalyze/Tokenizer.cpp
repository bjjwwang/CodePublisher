...
namespace Scanner
{
	...
	*/
	...
	public:
		...
		ConsumeState* nextState2(std::locale loc);
...
ConsumeState* ConsumeState::nextState()
{
	...
	if (_pContext->currChar == '\"' && _pContext->prevChar != '\\'){
		testLog("state: eatDQString");return _pContext->_pEatDQString;
	}
	if (_pContext->currChar == '\'' && _pContext->prevChar != '\\'){
		testLog("state: eatSQString");return _pContext->_pEatSQString;
	}
	if (_pContext->currChar == '/' && chNext == '/'){
		testLog("state: eatCppComment");return _pContext->_pEatCppComment;
	}
	if (_pContext->currChar == '/' && chNext == '*'){
		testLog("state: eatCComment");return _pContext->_pEatCComment;
	}
	if (_pContext->currChar == 'R' && chNext == '\"'){
		testLog("state: eatRawCppString");return _pContext->_pEatRawCppString;
	}
	if (_pContext->currChar == '@' && chNext == '\"'){
		testLog("state: eatRawCSharpString");return _pContext->_pEatRawCSharpString;
	}
	return nextState2(loc);
}
ConsumeState* ConsumeState::nextState2(std::locale loc)
{
	if (isOneCharToken(makeString(_pContext->currChar))) {
		testLog("state: eatSpecialCharacters"); return _pContext->_pEatSpecialCharacters;
	}
	if (std::isspace(_pContext->currChar, loc) && _pContext->currChar != '\n') {
		testLog("state: eatWhitespace"); return _pContext->_pEatWhitespace;
	}
	if (std::isalnum(_pContext->currChar, loc) || _pContext->currChar == '_') {
		testLog("state: eatAlphanum"); return _pContext->_pEatAlphanum;
	}
	if (ispunct(_pContext->currChar)) {
		testLog("state: eatPunctuator"); return _pContext->_pEatPunctuator;
	}
	if (!_pContext->_pIn->good()) {
		testLog("state: eatWhitespace"); return _pContext->_pEatWhitespace;
	}
