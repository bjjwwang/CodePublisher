...
====================
ver 4.0 : 07 Mar 2017
- And some new rules, to detect enum, typedef, using, etc;
...
namespace CodeAnalysis
{
	...
	public:
		...
		ASTNode* getGlobalScope() { return pGlobalScope; }//返回 Global Scope AST树 
	...
	public:
		bool doTest(const Scanner::ITokCollection* pTc) override
		{
			...
			if (se[0] == "enum") return IRule::Continue;
	...
	*/
	...
	public:
		...
		{
			Repository* pRepo = Repository::getInstance();
			if (pRepo->language() != Language::Cpp) return IRule::Continue;
			...
			if (tc.length() > 0 && (tc[0] == "using"||tc[0]=="typedef")) {
				doActions(pTc);
				return IRule::Stop;
			}
	...
	public:
		...
		void doAction(const Scanner::ITokCollection* pTc) override
		{
			...
			if (se[0] == "using"&& se[2]=="=" )
			{
				...
				GrammarHelper::showParse("alises declar", *pTc);
				return;
			}
			if (se[0] == "typedef")
			{
				declNode.declType_ = DeclType::typedefDecl;
				declNode.name_ = se[se.length() - 2];
				pCurrNode->decl_.push_back(declNode);
				GrammarHelper::showParse("typedef declar", *pTc);
				return;
			}
			if (GrammarHelper::isFunctionDeclaration(se, "parentNotFunction")){return;}
