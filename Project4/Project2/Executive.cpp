/////////////////////////////////////////////////////////////////////
// Executive.cpp - Directs Code Analysis and print                           //
// ver 1.6                                                         //
//-----------------------------------------------------------------//
...
// Author:      Jiawei Wang, Syracuse University, 2016 Fall MS     //
...
CodeAnalysisExecutive::CodeAnalysisExecutive()
{
	...
	Dependency = new DependencyAnalysis(Analysis->GetTable()->GetDb()); //get the dependency
...
bool CodeAnalysisExecutive::ProcessCommandLine(int argc, char* argv[])
{
	...
	try {
		...
		int i;
		if (argv[2][0] != '*') //it means that user specify the xml path
		{
			xmlpath_ = path_ + "\\" + argv[2];
			User_Specify_xml = true;
			std::cout << "\n  2. User specifies the XML output path, the path is \"" << xmlpath_ << "\" ";
			i = 3;
		}
		else
		{
			xmlpath_ = path_ + "\\" + "OutputXML.xml";
			std::cout << "\n  2. User do not specify the XML output path, the default path is \"" << xmlpath_ << "\" ";
			User_Specify_xml = false;
			i = 2;
		}
		for (; i < argc-1; ++i)
		{
			if (argv[i][0] == '*')
				...
				openfiles_.push_back(argv[i]);
		}
		HtmlPath_ = FileSystem::Path::getFullFileSpec(argv[argc-1]);
		if (patterns_.size() == 0)
		{
			showUsage();
			return false;
...
//first read to process the AST Tree and create type table for furthur use.
void CodeAnalysisExecutive::firstRead()
{
	ASTNode* pGlobalScope = pRepo_->getGlobalScope();
	Analysis->DoAnalysis(pGlobalScope);
	Dependency->CreateGraph();
}

//second read to create graph, analyze dependencies and find strong components
void CodeAnalysisExecutive::secondRead()
{
	using namespace Scanner;
	std::vector<File> hfiles = cppHeaderFiles();
	std::vector<File> cppfiles = cppImplemFiles();
	std::vector<File> csfiles = csharpFiles();
	std::vector<File> allfiles;

	for (File cpp : hfiles)allfiles.push_back(cpp);
	for (File cpp : cppfiles)allfiles.push_back(cpp);
	for (File cpp : csfiles)allfiles.push_back(cpp);
	for (File file : allfiles)
	{
		...
		do
		{
			...
			Dependency->DetectDependency(tok, file);
		} while (in.good());
	}
	Dependency->PersistXML(xmlpath_);  //save dependency xml
	scc = new StrongComponents<std::string, std::string>(Dependency);
	scc->FindSSC();
	scc->SccToXml(xmlpath_);  //save strong components to xml
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		test.Pr3(exec.HtmlPath());  //execute project 3 requirements
