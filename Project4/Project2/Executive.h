...
*
*  ver 1.6 : 03 Apr 2017
*  - add open_files for user to specify the files to open in browser.
*  ver 1.5 : 06 Mar 2017
*  - add first read and second read
*  - add req1() - req9() in executive.h
*  ver 1.4 : 26 Feb 2016
...
namespace CodeAnalysis
{
	...
	public:
		...
		void firstRead();
		void secondRead();
		ASTNode * GetAst() { return pRepo_->getGlobalScope(); }
		TypeTable* GetTable() { return Analysis->GetTable(); }
		DependencyAnalysis* GetDependency() { return Dependency; }
		StrongComponents<std::string, std::string>* GetSCC() { return scc; }
		Path Getpath() { return path_; }
		Path GetXmlPath() { return xmlpath_; }
		Files GetOpenFiles() { return openfiles_; } // the user specified files to open
		bool IfUserSpecify() { return User_Specify_xml; }
		std::string& HtmlPath() { return HtmlPath_; }
	...
	private:
		...
		Path path_;
		Path xmlpath_;
		bool User_Specify_xml;

		Files openfiles_; //for the files that will be open 
		Patterns patterns_;
		Options options_;
		FileMap fileMap_;
		...
		TypeAnalysis* Analysis = nullptr;
		DependencyAnalysis* Dependency = nullptr;
		StrongComponents<std::string, std::string>* scc = nullptr;
